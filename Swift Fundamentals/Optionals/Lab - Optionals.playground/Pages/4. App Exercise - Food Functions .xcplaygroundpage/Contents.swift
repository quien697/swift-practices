...
func LoggedMeals(meal: String) -> Meal? {
  return meals[meal]
}
if let logged = LoggedMeals(meal: "Breakfast") {
  print(logged)
} else {
  print("The user hasn't logged that meal yet.")
}
if let logged = LoggedMeals(meal: "Dinner") {
  print(logged)
} else {
  print("The user hasn't logged that meal yet.")
}
